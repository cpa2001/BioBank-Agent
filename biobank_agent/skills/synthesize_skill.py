"""Live paper → skill synthesis (M12), through the safety gates.
...
it runs the M11 methodology pre-gate (reject consensus statistical/omics sins before any code lands),
the M13 load-safety validator (reject unsafe imports / import-time side effects), then applies the
...
def synthesize_skill_from_paper(skill_name, skill_code, contract=None, summary="", paper_source="", *, ctx=None) -> dict:
    ...
    # M13 load gate BEFORE any apply: refuse unsafe imports / import-time side effects.
