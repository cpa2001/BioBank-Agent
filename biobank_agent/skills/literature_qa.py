...
Integration:
    Citations returned by this skill feed into the Evidence Lattice
    (biobank_agent/evidence.py) for claim-level provenance tracking.
"""
...
# ── Utility for Evidence Lattice integration ─────────────────────────────────


def citations_to_evidence_nodes(
    result: dict[str, Any],
    claim_text: str = "",
) -> list[dict[str, Any]]:
    """Convert literature_qa citations into Evidence Lattice node format.

    Use this to feed PaperQA2 results into the Evidence Lattice
    (biobank_agent/evidence.py) for claim-level provenance tracking.

    Args:
        result: Output dict from literature_qa()
        claim_text: The claim being supported by these citations

    Returns:
        List of dicts suitable for EvidenceNode creation
    """
    ...
    for citation in result.get("citations", []):
        node = {
            "evidence_type": "paper",
            "source_ref": citation.get("doi") or citation.get("title", "unknown"),
            "source_hash": hashlib.sha256(
                citation.get("passage", "").encode()
            ).hexdigest()[:16],
            "passage": citation.get("passage", ""),
            "confidence": citation.get("confidence", 0.5),
            "direction": "supports",  # Default; can be overridden by entailment check
        }
        nodes.append(node)
