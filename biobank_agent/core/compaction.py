...
3. ``before_last_user_message`` — codex-style context-window
...
def before_last_user_message(
    ...
    *,
    ...
    """Codex-style: replace pre-last-user-turn content with a summary.
...
def force_aggressive(messages: list[dict], *, keep_last_n_tool_pairs: int = 2) -> list[dict]:
    ...
    Codex review note (BLOCKER #1): the previous implementation cut off
    at the most recent user turn even when the runtime had already
    produced fresh assistant tool_calls + tool results in the current
    round; this would silently drop active context. We now keep the
    *entire* tail (>= last user turn) verbatim.
    """
