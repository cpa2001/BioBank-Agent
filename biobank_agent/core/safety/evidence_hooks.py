"""Auto-link tool outputs into the Action Graph as evidence.

Earlier agent versions returned ``fetch_paper`` / ``literature_qa`` /
``read_paper`` data into the ReAct loop without recording the
claim ↔ evidence link anywhere.
...
returns. Evidence linking is observability, not load-bearing.
"""
