...
def load_checkpoint(
    ...
    *,
    ...
    Accepts three id forms (the project has two id systems and CLI
    users shouldn't have to pick the right one):
...
def replay(
    *,
    ...
    if saved_fp is None:
        # A missing saved fingerprint must refuse, not be treated as soft
        # drift. The whole point of /replay is to prove data identity —
        # without a saved fingerprint we cannot.
        return {
            "status": "refused",
            "provenance_id": provenance_id,
            "skill": skill_name,
