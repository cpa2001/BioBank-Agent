...
class TestReportImport:
    ...
    @patch("biobank_agent.skills.report._write_html", return_value=None)
    def test_dual_report_handles_phenotype_group_cohort_without_label_column(self, mock_html, tmp_path):
        """WGS phenotype cohorts are not coded-diagnosis case/control cohorts."""
        from biobank_agent.skills.report import generate_report
        import pandas as pd

        cohort = pd.DataFrame({
            "sample_id": ["J1", "J2", "V1"],
            "phenotype_group": ["J", "J", "V"],
            "age": [41, 35, 55],
        })
        ctx = _make_ctx(
            tmp_path,
            records=[
                _make_record("cohort_phenotype_summary", key_results={"n_total": 3, "n_groups": 2}),
                _make_record("vcf_association", key_results={
                    "case_group": "J",
                    "control_group": "V",
                    "n_cases": 2,
                    "n_controls": 1,
                    ...
                    "lambda_gc": None,
                    "power_warning": "Exploratory only.",
                }),
            ],
            cohorts={"phenotype_phenotype_group": cohort},
        )
        ctx.settings.biobank_name = "VirtualCell WGS Cohort"
        ctx.settings.biobank_description = "a 28-sample WGS cohort"

        result = generate_report(title="WGS", format="dual", ctx=ctx)

        assert result["format"] == "dual"
        nature = (ctx.report_dir / "report_nature.md").read_text()
        assert "WGS samples were available across phenotype groups" in nature
