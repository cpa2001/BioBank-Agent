...
def test_force_review_branch_keeps_allow_listed_change_off_main(tmp_path):
    repo = _init_repo(tmp_path)
    # Identical to the auto-merge case, but force_review_branch (agent-synthesized skills)
...
def test_mutation_check_flags_gaming_vectors():
    # Static red flags before the worktree apply.
...
def test_apply_rejects_test_deleting_mutation(tmp_path):
    # A self-edit that deletes a test to pass the gate is rejected outright,
